...
glm::fquat MadgwickAHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my,
	...
	if((mx == 0.0f) && (my == 0.0f) && (mz == 0.0f)) {
		return MadgwickAHRSupdateIMU(gx, gy, gz, ax, ay, az, sample_freq);
	}
...
glm::fquat MadgwickAHRSupdateIMU(float gx, float gy, float gz, float ax, float ay, float az,
								 float sample_freq) {
	float recipNorm;
	float s0, s1, s2, s3;
	float qDot1, qDot2, qDot3, qDot4;
