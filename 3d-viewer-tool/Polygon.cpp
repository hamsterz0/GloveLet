...
Polygon::~Polygon() {
    ...
    while(next != end_vertex) {
        std::cout << "start= " << start_vertex << ", next= " << next << ", next_next= " << next->getNextVertex()
                  << ", end_vertex= " << end_vertex << std::endl;
        std::cout << "next->pos= (" << next->getPos().x << ", " << next->getPos().y << ", " << next->getPos().z << ")"
                  << ", next_next->pos= (" << next->getNextVertex()->getPos().x << ", " << next->getNextVertex()->getPos().y << ", " << next->getNextVertex()->getPos().z << ")"
                  << ", end->pos= (" << end_vertex->getPos().x << ", " << end_vertex->getPos().y << ", " << end_vertex->getPos().z << ")" << std::endl;
        prev = next;
